...
class BaselineTrainer:
    ...
    def get_logistic_regression(self):
        model = LogisticRegression(penalty='l2', C=1.0, solver='lbfgs', max_iter=1000, tol=1e-4, random_state=42)
        pipeline = Pipeline([('scaler',StandardScaler()), ('model',model)])
        return self._evaluate_and_print("Logistic Regression", pipeline)

    def get_ridge(self):
        model = RidgeClassifier(alpha=1.0, solver='auto', max_iter=1000, tol=1e-4, random_state=42)
        pipeline = Pipeline([('scaler',StandardScaler()), ('model',model)])
        return self._evaluate_and_print("Ridge Classifier", pipeline)

    def get_lasso(self):
        model = LogisticRegression(penalty='l1', C=1.0, solver='liblinear', max_iter=1000, random_state=42)
        pipeline = Pipeline([('scaler',StandardScaler()), ('model',model)])
        return self._evaluate_and_print("Lasso (L1 Logistic)", pipeline)

    def get_elastic_net(self):
        model = LogisticRegression(penalty='elasticnet', C=1.0, l1_ratio=0.5, solver='saga', max_iter=1000, random_state=42)
        pipeline = Pipeline([('scaler',StandardScaler()), ('model',model)])
        return self._evaluate_and_print("Elastic Net", pipeline)
    ...
    def get_knn(self):
        model = KNeighborsClassifier(n_neighbors=5, weights='uniform', algorithm='auto', leaf_size=30, p=2, n_jobs=-1)
        return self._evaluate_and_print("KNN", model)

    def get_svm(self):
        model = SVC(C=1.0, kernel='rbf', degree=3, gamma='scale', probability=False, random_state=42)
        return self._evaluate_and_print("SVM", model)
...
if __name__ == "__main__":
    ...
    X_train['Is_Tyre_Old_Risk'] = (X_train['TyreLife'] > 2).astype(int) # Note: Sqrt hataya ba toh check threshold!
    ...
    new_column_names = encoder.get_feature_names_out(cols_to_encode)
    train_cols = X_train.columns
    train_encoded_df = pd.DataFrame(encoded_train,columns=encoder.get_feature_names_out(cols_to_encode),index=X_train.index)
    test_encoded_df = pd.DataFrame(encoded_test,columns=encoder.get_feature_names_out(cols_to_encode),index=X_test.index)
    X_train = pd.concat([X_train.drop(columns = cols_to_encode),train_encoded_df],axis = 1)
    X_test = pd.concat([X_test.drop(columns = cols_to_encode),test_encoded_df],axis = 1)
    ...
    auc_score = roc_auc_score(y_test, y_pred_proba_er) # PROBABILITY use kiya, zero_division hata diya
...
df_final['Stint_Fatigue'] = df_final['Stint'] * df_final['LapNumber']
df_final['Pace_Efficiency'] = df_final['Stint'] / (df_final['LapTime (s)'] + 0.001)
df_final['Tire_Danger_Index'] = df_final['Cumulative_Degradation'] * df_final['TyreLife']

df_final = df_final.drop(columns=['id', 'Driver','RaceProgress'], errors='ignore')

df_final = df_final.dropna()

# SAME compound mapping from TRAIN ONLY
df_final['Compound_Pit_Probablity'] = df_final['Compound'].map(compound_pit_proba)
df_final['Z'] = df_final['Compound_Pit_Probablity'] * df_final['LapTime_Delta']

df_final['Is_Tyre_Old_Risk'] = (df_final['TyreLife'] > 2).astype(int)
df_final['Critical_Pit_Alarm'] = df_final['Is_Tyre_Old_Risk'] * df_final['LapTime_Delta']
